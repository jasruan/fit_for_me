...
class ChestArmCollectionVC: UICollectionViewController {
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let dest = segue.destination as! ExerciseCollectionVC
            ...
            case 0:
                dest.currentExercises = Exercises.shared.bicepEx
                //dest.currImages = bicepImages
                break
            case 1:
                dest.currentExercises = Exercises.shared.tricepEx
                //dest.currImages = tricepImages
                break
            case 2:
                dest.currentExercises = Exercises.shared.shouldersEx
                //dest.currImages = shouldersImages
                break
            case 3:
                dest.currentExercises = Exercises.shared.foreEx
                //dest.currImages = foreImages
                break
            case 4:
                dest.currentExercises = Exercises.shared.chestEx
               // dest.currImages = chestImages
                break
            default:
                break
            }
    ...
    func loadData(){
        guard let pathTwo = Bundle.main.url(forResource: "arms", withExtension: "js")else{
            print("Error: could not find arms.js!")
            return
        }
        do{
    ...
    func parse(json:JSON){
        let array = json["arms"].arrayValue
        for c in array{
            let name = c["name"].stringValue
            let equip = c["equip"].stringValue
            let url = c["id"].stringValue
            let type = c["muscle"].stringValue
            let image = c["image"].stringValue
            let exercises = Exercise(title: name, videoId: url, equip: equip, type: type, imageId:image)
            //add elements to array and then sort it out afterwards
            currEx.append(exercises)
            Exercises.shared.bicepEx = currEx.filter(){ curr in
                (curr.muscle?.contains("bicep"))!
            }
            Exercises.shared.tricepEx = currEx.filter(){
                curr in (curr.muscle?.contains("triceps"))!
            }
            Exercises.shared.shouldersEx = currEx.filter(){
                curr in (curr.muscle?.contains("shoulder"))!
            }
            Exercises.shared.foreEx = currEx.filter(){
                curr in (curr.muscle?.contains("forearms"))!
            }
            Exercises.shared.chestEx = currEx.filter(){
                curr in (curr.muscle?.contains("chest"))!
            }
